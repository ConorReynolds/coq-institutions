...
Record Algebra (Σ : Signature) := {
  ...
  interp_funcs (F : Funcs Σ) : HList interp_sorts (ar F) -> interp_sorts (res F) ;
  interp_preds (P : Preds Σ) : HList interp_sorts (arP P) -> Prop ;
}.
...
Lemma eq_algebra Σ (A A' : Algebra Σ)
  ...
  (q : rew [λ interp, ∀ F, HList interp (ar F) -> interp (res F)] p
        in (@interp_funcs Σ A) = @interp_funcs Σ A')
  (r : rew [λ interp, ∀ P, HList interp (arP P) -> Prop] p
...
| term : ∀ F : Funcs Σ, HList Term (ar F) -> Term (res F).
...
(* Need this for derived signature morphisms *)
Definition TaggedTerm Σ : Tagged (list (Sorts Σ) * Sorts Σ) := {|
  tagged_data := { x & Term Σ (fst x) (snd x) } ;
  get_tag := λ t, projT1 t ;
|}.
...
Record SignatureMorphismᵈ (Σ Σ' : Signature) : Type := {
  ...
  on_funcsᵈ : tagged_morphism (lift_ty on_sortsᵈ) (Funcs Σ) (TaggedTerm Σ') ;
  on_predsᵈ : tagged_morphism (map on_sortsᵈ) (Preds Σ) (Preds Σ') ;
}.

Arguments on_sortsᵈ {Σ Σ'} σ s : rename.
Arguments on_funcsᵈ {Σ Σ'} σ : rename.
Arguments on_predsᵈ {Σ Σ'} σ : rename.

Definition reindex_arity [Σ Σ'] [A' : Algebra Σ'] (σ : SignatureMorphism Σ Σ') {F} :
    HList (A' ∘ σ) (ar F) -> HList A' (ar (on_funcs σ F)).
  intros.
  pose (reindex σ X).
...
Definition ReductAlgebra (A' : Algebra Σ') : Algebra Σ.
  refine {|
    ...
    interp_funcs := λ F θ, rew _ in interp_funcs A' (on_funcs σ F) (rew _ in reindex σ θ) ;
    interp_preds := λ P θ, interp_preds A' (on_preds σ P) (rew _ in reindex σ θ) ;
...
Fixpoint reindex_member
    ...
    : member (f j) (map f js) :=
  match m with
  | @HZ _ _ js       => @HZ _ (f j) (map f js)
  | @HS _ _ j' js m' => @HS _ (f j) (f j') (map f js) (reindex_member f m')
  end.
...
Equations? on_terms
  ...
  on_terms σ (term F args) :=
    rew _ in term (on_funcs σ F)
      (rew [λ s, HList (Term Σ' (map σ Γ)) s] _ in map_on_terms σ args) }
...
Equations eval_term {Σ : Signature} (A : Algebra Σ)
                    ...
                    : HList (interp_sorts A) Γ -> interp_sorts A s := {
  eval_term A (var i) env := HList.nth env i ;
  eval_term A (term F args) env := interp_funcs A F (map_eval_term A args env) }
where map_eval_term {Σ : Signature} (A : Algebra Σ)
                    {Γ : Ctx Σ} {w : Arity Σ} (args : HList (Term Σ Γ) w)
                    : HList (interp_sorts A) Γ -> HList (interp_sorts A) w := {
  map_eval_term A ⟨⟩ env := ⟨⟩ ;
...
Lemma map_eval_term_hmap
    ...
    (args : HList (Term Σ Γ) w) (env : HList (interp_sorts A) Γ) :
...
where map_on_termsᵈ
    ...
    : HList (Term Σ Γ) w -> HList (Term Σ' (map σᵈ Γ)) (map σᵈ w) := {
  map_on_termsᵈ σᵈ ⟨⟩ := ⟨⟩ ;
...
Class AlgHom (h : ∀ s, A s -> B s) := 
  alg_hom_commutes : ∀ (F : Funcs Σ) (args : HList A (ar F)),
